...
class ScrapeGatwickFlights extends Command
{
    ...
    {
        $this->arrival_flights_scrapping();         // Calling the function to scrape arrival flights and save them to the database
        // $this->departure_flights_scrapping();
    }

    /* For now we only need to save the arrival flights, but we can use this function to scrape the departure flights if needed in the future. */
    private function arrival_flights_scrapping(){
        ...
        try {
            ...
            do {
                ...
                if ($response->successful()) {
                    ...
                    $crawler->filter('.flight-line')->each(function ($node) use (&$f_Data, &$totalFound) {
                        $flightData = [
                            ...
                            'status' => $node->filter('.d-none.d-md-table-cell')->eq(1)->text(''),
                        ];

                        if (!empty($flightData['flight_no'])) {
                            \App\Models\Flight::updateOrCreate(['flight_no' => $flightData['flight_no'], 'time' => $flightData['time'], 'status' => $flightData['status']], $flightData);
                            $f_Data[] = $flightData;
                            $totalFound++;
                        }
                    });
                ...
                sleep(5); // avoid overwhelming the server
            } while (!empty($f_Data));
    ...
    /* For now we don't need to save the departure flights, but we can use this function to scrape them if needed in the future. */
//     private function departure_flights_scrapping(){
//         $this->info('Starting Gatwick flights scraping...');
